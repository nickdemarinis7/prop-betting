...
Reads predictions_points_YYYYMMDD.csv (current format) — falls back to
...
def validate_date(date_str, gamelog_scraper, all_players, quiet=False):
    ...
    new_file = f"predictions_points_{date_str}.csv"
    ...
    for _, row in predictions.iterrows():
        ...
        actual = int(matched['PTS'])
        diff = round(actual - float(projection), 1)
        l10 = row.get('pts_last10_avg', projection)
        try:
        ...
        for col in ['prob_15+', 'prob_20+', 'prob_25+', 'prob_30+']:
            if col in row.index:
        ...
        if not quiet:
            ...
            print(f"   {icon} {player_name:25s} Proj:{float(projection):5.1f} "
                  f"Act:{actual:3d} Diff:{diff:+5.1f}")
...
def print_summary(df, label=""):
    ...
    print(f"   Bias:   {bias:+.2f} pts ({'over' if bias < 0 else 'under'}-projecting)")

    w3 = (df['Diff'].abs() <= 3).mean()
    w5 = (df['Diff'].abs() <= 5).mean()
    w8 = (df['Diff'].abs() <= 8).mean()
    print(f"\n📏 Accuracy:")
    ...
    if 'Baseline_Diff' in df.columns:
        ...
        if len(valid) > 0:
            ...
            print(f"   Baseline MAE: {base_mae:.2f} pts")
            print(f"   Model MAE:    {mae:.2f} pts")
            print(f"   {icon} Model is {abs(improvement):.2f} pts "
                  f"{'better' if improvement > 0 else 'worse'} ({abs(pct):.0f}%)")

    # By projection bucket
    print(f"\n📊 By Projection Range:")
    for lo, hi, lbl in [(0, 12, '<12'), (12, 18, '12-18'),
                         (18, 25, '18-25'), (25, 50, '25+')]:
...
def print_individual(df):
    ...
    for _, row in sorted_df.iterrows():
        ...
        print(f"{icon} {row['Player']:25s} | Proj:{proj:5.1f} | "
              f"Act:{actual:3d} | Diff:{diff:+6.1f} {conf_icon}{playoff_tag}{flag_str}")
...
if args.cumulative:
    all_files = (
        glob.glob('predictions_points_*.csv')
    ...
    for pf in all_files:
        ...
        ds = (
            base.replace('predictions_points_', '')
                .replace('predictions_production_', '')
                .replace('.csv', '')
        )
        # Prefer the new filename when both exist
        if ds not in by_date or 'predictions_points_' in base:
            by_date[ds] = pf
